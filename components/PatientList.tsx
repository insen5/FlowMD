...
import { ChevronRight, Search, Clock, TrendingUp, TrendingDown, Minus, X, Activity, Calendar, History, ShieldCheck, ArrowRight, Zap, Target } from 'lucide-react';
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 text-[10px] text-slate-400 font-bold uppercase tracking-tight">
                    ...
                    <span className="text-blue-500 font-black">#P-0{p.id}</span>
                  </div>
                </div>
              </div>
        ...
        <div className="fixed inset-0 z-[60] bg-slate-900/40 backdrop-blur-xl flex items-end sm:items-center justify-center p-4 animate-in fade-in duration-300">
          <div className="bg-white w-full max-w-xl rounded-[3rem] shadow-2xl overflow-hidden flex flex-col max-h-[90vh] animate-in slide-in-from-bottom-10">
            ...
            <div className="p-8 overflow-y-auto no-scrollbar space-y-8">
              <div className={`p-6 rounded-[2rem] border-2 flex items-center justify-between ${getTrendStyles(detailedPatient.trend).bg} ${getTrendStyles(detailedPatient.trend).border}`}>
                <div className="flex items-center gap-4">
                  <div className={`p-3 rounded-2xl bg-white shadow-sm ${getTrendStyles(detailedPatient.trend).color}`}>
                    {React.createElement(getTrendStyles(detailedPatient.trend).icon, { size: 24 })}
                  </div>
                  <div>
                    <div className="text-[9px] font-black uppercase tracking-widest text-slate-400 mb-0.5">Clinical Trend</div>
                    <div className={`font-black text-lg ${getTrendStyles(detailedPatient.trend).color}`}>{getTrendStyles(detailedPatient.trend).label}</div>
                  </div>
                </div>
              </div>
